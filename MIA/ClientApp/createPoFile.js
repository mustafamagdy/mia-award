...
if (args.length == 0 || args.indexOf("-f") == -1) {
  console.error("Excel file name not specified, use -f filename.xls");
  return;
} else {
  const _fileArgIndex = args.indexOf("-f");
  if (_fileArgIndex != -1) {
    outputFile = args[_fileArgIndex + 1];
  }
}
