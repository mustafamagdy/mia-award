...
namespace MIA.Administration.Middlewares {
  /*
   * Author: https://bitbucket.org/paddo-org/image-resizing/src/master/
   * Modified to fit our needs
  */
  public class ImageProxyMiddleware {
    public const string CACHED_IMAGE_DIR = "cached_images";
    struct ResizeParams {
    ...
    public ImageProxyMiddleware(RequestDelegate next,
      ...
      ILogger<ImageProxyMiddleware> logger,
    ...
    public async Task Invoke(HttpContext context) {
      ...
      // /r/abcdxyz?
      var imageId = path.Value.Substring(path.Value.LastIndexOf('/') + 1);

      // extract resize params if exists
      var resizeParams = GetResizeParams(path, context.Request.Query);

      var imageDir = Path.Combine(_env.WebRootPath, CACHED_IMAGE_DIR);
      if (!Directory.Exists(imageDir)) {
        Directory.CreateDirectory(imageDir);
      }

      // get the image location on disk
      var imagePath = Path.Combine(imageDir, $"{imageId}_{resizeParams.FileInfo()}");
      ...
      {
        var imageFromDb = await _db.Images.FindAsync(imageId);
        if (imageFromDb == null) {
          await _next.Invoke(context);
          return;
        }

        if (imageFromDb.Data.Length == 1) {
          //not valid image
          await _next.Invoke(context);
          return;
        }

        await File.WriteAllBytesAsync(imagePath, imageFromDb.Data);
      }
    ...
    Regex imagePathRegex = new Regex(@"\/r\/.*");
    private bool IsImagePath(PathString path) {
      if (path == null || !path.HasValue)
        return false;

      return imagePathRegex.IsMatch(path.Value);
    }
    ...
    private ResizeParams GetResizeParams(PathString path, IQueryCollection query) {
      ...
      else if (path.Value.LastIndexOf('.') != -1)
