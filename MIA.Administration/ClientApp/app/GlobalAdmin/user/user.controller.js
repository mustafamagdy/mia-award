(function () {
  ...
  function userController(
    ...
    function refreshUsers() {
      blockUI.start("Loading...");
      var k = UserResource.getAllUsersByUserType({
    ...
    vm.ChangeStatus = function (model, index) {
      var updateObj = new UserResource();
      updateObj.userId = model.id;
      updateObj.$toggleStatus({ userId: model.id }).then(
        function (data, status) {
          vm.userList[index].active = !vm.userList[index].active;
          ToastService.show(
            "right",
            "bottom",
            "fadeInUp",
