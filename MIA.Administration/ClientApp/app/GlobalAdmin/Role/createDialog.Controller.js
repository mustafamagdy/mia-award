(function () {
  ...
  angular
    ...
    .controller("createRoleDialogController", [
      ...
      "ToastService",
      "$rootScope",
      "PermissionPrepService",
      createRoleDialogController,
    ]);

  function createRoleDialogController(
    $scope,
    blockUI,
    $http,
    $state,
    ...
    vm.ChangeSelectedModule = function () {
      angular.forEach(vm.selectedModule, function (value, key) {
        angular.forEach(value.permessions, function (valuePermission, key1) {
          ...
          } else vm.selectedModuleList.push(valuePermission);
        });
      });
    };
    ...
    vm.AddNewRole = function () {
      ...
      newObj.roleName = vm.roleName;
      newObj.$create({roleName: vm.roleName}).then(
